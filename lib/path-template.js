...
PathTemplate.variables = function (template) {
  var acc = [],
    ...
    type,
    val;
  for (var i = 0; i < templateLength; i += 1) {
    part = template[i];
    type = part[0];
    val = part[1];
    if (type === "variable") {
...
var copyCaptures = function(captures) {
...
PathTemplate.match = function (template, string) {
    var matchers = [{template: template, index: 0, captures: {}}];
    var parts = tokenize(string);

  for (var i = 0; i < parts.length; i += 1) {
    matchers = flattenCompact(matchers.map(function(m) {
      return advanceMatcher(m, parts[i])
    }));
...
var range = function (start, end, exclusive) {
  var acc = [];
  for (var i = start; exclusive ? i < end : i <= end; i += 1) {
    acc.push(i);
  }
  return acc;
};
